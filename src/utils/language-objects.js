export const navEng = [
  { name: 'About me', color: 'red' },
  { name: 'Skills', color: 'orange' },
  { name: 'Projects', color: 'yellow' },
  { name: 'Contacts', color: 'green' },
  { name: 'CV', color: 'blue' },
];
export const navUa = [
  { name: 'Про мене', color: 'red' },
  { name: 'Навички', color: 'orange' },
  { name: 'Проекти', color: 'yellow' },
  { name: 'Контакти', color: 'green' },
  { name: 'Резюме', color: 'blue' },
];
export const qualitiesEng = [
  { name: 'Team player', icon: '/pokemon-icons/squirtle-icon.png' },
  { name: 'Responsible', icon: '/pokemon-icons/bulbasaur-icon.png' },
  { name: 'Communicative', icon: '/pokemon-icons/jigglypuff-icon.png' },
  { name: 'Quick learner', icon: '/pokemon-icons/meowth-icon.png' },
  { name: 'Likes order', icon: '/pokemon-icons/charmander-icon.png' },
];

export const frontSkills = [
  { icon: '/empty.png ', name: '', iconStyle: 'empty', id1: 'f1', id2: 'ff1' },
  { icon: '/empty.png', name: '', iconStyle: 'empty', id1: 'f2', id2: 'ff2' },

  {
    icon: '/skills-icons/redux-icon.svg',
    name: 'Redux',
    iconStyle: 'circled',
    id1: 'f9',
    id2: 'ff9',
  },
  {
    icon: '/skills-icons/nextjs-icon.png',
    name: 'Next.js',
    iconStyle: 'circled',
    id1: 'f8',
    id2: 'ff8',
  },
  {
    icon: '/skills-icons/react-icon.png',
    name: 'React',
    iconStyle: 'circled',
    id1: 'f7',
    id2: 'ff7',
  },
  {
    icon: '/skills-icons/sass-icon.png',
    name: 'Sass/Scss',
    iconStyle: 'circled',
    id1: 'f6',
    id2: 'ff6',
  },
  {
    icon: '/skills-icons/js-icon.png',
    name: 'Java Script',
    iconStyle: 'circled',
    id1: 'f5',
    id2: 'ff5',
  },
  {
    icon: '/skills-icons/css-icon.png',
    name: 'CSS',
    iconStyle: 'circled',
    id1: 'f4',
    id2: 'ff4',
  },
  {
    icon: '/skills-icons/html-icon.png',
    name: 'HTML',
    iconStyle: 'circled',
    id1: 'f3',
    id2: 'ff3',
  },
];
export const endSkills = [
  { icon: '/empty.png ', name: '', iconStyle: 'empty', id1: 'e1', id2: 'ee1' },
  { icon: '/empty.png', name: '', iconStyle: 'empty', id1: 'e2', id2: 'ee2' },
  { icon: '/empty.png', name: '', iconStyle: 'empty', id1: 'e3', id2: 'ee3' },
  { icon: '/empty.png', name: '', iconStyle: 'empty', id1: 'e4', id2: 'ee4' },
  { icon: '/empty.png', name: '', iconStyle: 'empty', id1: 'e5', id2: 'ee5' },
  { icon: '/empty.png', name: '', iconStyle: 'empty', id1: 'e6', id2: 'ee6' },
  { icon: '/empty.png', name: '', iconStyle: 'empty', id1: 'e7', id2: 'ee7' },
  {
    icon: '/skills-icons/nodejs-icon.png',
    name: ' Node.js',
    iconStyle: 'circled',
    id1: 'e8',
    id2: 'ee8',
  },
  {
    icon: '/skills-icons/nextjs2-icon.png',
    name: 'Next.js API',
    iconStyle: 'circled',
    id1: 'e9',
    id2: 'ee9',
  },
];
export const otherSkills = [
  { icon: '/empty.png', name: '', iconStyle: 'empty', id1: 'o1', id2: 'oo1' },
  { icon: '/empty.png', name: '', iconStyle: 'empty', id1: 'o2', id2: 'oo2' },
  { icon: '/empty.png', name: '', iconStyle: 'empty', id1: 'o3', id2: 'oo3' },
  {
    icon: '/empty.png',
    name: '',
    iconStyle: 'empty',
    id1: 'o8',
    id2: 'oo8',
  },
  {
    icon: '/empty.png',
    name: '',
    iconStyle: 'empty',
    id1: 'o6',
    id2: 'oo6',
  },
  {
    icon: '/skills-icons/bootstrap-icon.png',
    name: 'Bootstrap',
    iconStyle: 'circled',
    id1: 'o4',
    id2: 'oo4',
  },
  {
    icon: '/skills-icons/eslint-icon.png',
    name: 'ESLint',
    iconStyle: 'circled',
    id1: 'o5',
    id2: 'oo5',
  },
  {
    icon: '/skills-icons/postman-icon.png',
    name: 'Postman',
    iconStyle: 'circled',
    id1: 'o7',
    id2: 'oo7',
  },

  {
    icon: '/skills-icons/git-icon.png',
    name: 'Git + Git Bash',
    iconStyle: 'circled',
    id1: 'o9',
    id2: 'oo9',
  },
];
